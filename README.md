...
- [jolokia-mcp-server-0.5.2-runner.jar](https://github.com/jolokia/jolokia-mcp-server/releases/download/v0.5.2/jolokia-mcp-server-0.5.2-runner.jar)
...
```json
{
  "mcpServers": {
    "jolokia": {
      ...
      "args": [
        "-jar",
        "<path-to-the-runner-jar>/jolokia-mcp-0.5.2-runner.jar"
      ]
    }
  }
}
...
```json
{
  "mcpServers": {
    "jolokia": {
      "command": "jbang",
      "args": [
        "org.jolokia.mcp:jolokia-mcp-server:0.5.2:runner"
      ]
    }
  }
}
...
- [jolokia-mcp-agent-jvm-0.5.2-javaagent.jar](https://github.com/jolokia/jolokia-mcp-server/releases/download/v0.5.2/jolokia-mcp-agent-jvm-0.5.2-javaagent.jar)

Then run your Java application with `-javaagent` option:

```console
java -javaagent:jolokia-mcp-agent-jvm-0.5.2-javaagent.jar -jar your-app.jar
```
...
```console
java -jar jolokia-mcp-server-0.5.2-runner.jar [Jolokia URL]
```

Using JBang, you can directly run it with the Maven GAV (`org.jolokia.mcp:jolokia-mcp-server:0.5.2:runner`):

```console
jbang org.jolokia.mcp:jolokia-mcp-server:0.5.2:runner
```
...
```console
java -jar jolokia-mcp-server-0.5.2-runner.jar --http
```
